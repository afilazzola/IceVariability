...
firstStack <- stack(iceOnFiles, bands=i)
...
colors <- rev(colorspace::sequential_hcl(9, palette = "Dark Mint"))
breakpoints <- seq(0,40, length.out = 9) ## Set breakpoints

sdTimersies <- foreach(i = 1:199, .combine=stack, .packages=c("raster","ncdf4","dplyr")) %dopar% {
## Load first year from all rasters for ice on
...
png(paste("iceGif//SDmodel",i+1900,".png"))
...
library(magick)
list.files(path='iceGif', pattern = '*.png', full.names = TRUE) %>% 
        image_read() %>% # reads each path file
        image_join() %>% # joins image
        image_animate(fps=10) %>% # animates, can opt for number of loops
        image_write("IceChange.gif") # write to current di
